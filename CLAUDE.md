...
| Outcome accounting | **Three-way at the source (corrected 05-Aug-2026): every labelled row stores `outcome_*` ∈ {target, stop, timeout} and the realised `r_*`.** The binary label is unchanged (+1 only on target-before-stop) so the classifier contract is identical, but expectancy must be measured from the stored R — a timeout is marked to its actual exit, never to −1R. The old `where(win, R, −1)` charged every non-win a full stop-out, which biased every reported expectancy pessimistic: swing's 0.274 base rate sat well below the 0.385 a driftless random walk gives on these barriers, which is the signature of timeouts booked as losses. Changing this **requires rebuilding the feature parquets** — the columns are stored, never inferred downstream. | same |
...
- **Calibration:** isotonic regression fit on out-of-fold predictions. The UI only ever sees the calibrated probability. **The calibration curve must be fit and evaluated on DISJOINT slices of the out-of-fold data (`calibration_curve_oos`), or it proves nothing** — corrected 05-Aug-2026: the original curve applied the isotonic to the very rows it was fitted on, and since isotonic's fitted value for a block *is* that block's mean outcome, it returned x == y at every point by construction. It could not have revealed a miscalibrated model even in principle, which made it a false assurance rather than merely a useless one. The in-sample curve may still be shown for shape, but it is labelled as such and never presented as validation. If the out-of-sample curve cannot be computed (too few rows), report its absence — never substitute the flattering one.
- **Calibration ceilings need evidence:** a regime bucket is used only if its TOP isotonic block holds ≥ `MIN_TOP_BLOCK_N` samples. `MIN_BUCKET_N` guards a bucket's total size but says nothing about the block that sets its ceiling — and the ceiling is the only part the emission gate ever reads. A bucket failing this is rejected, falls back to the pooled map, and the rejection is recorded on the model and printed (never silent); a degenerate *pooled* map raises `DegenerateCalibration` outright, since it has nothing to fall back to.
...
src/
  ...
  jobs/      scheduler.py (APScheduler): in-market intraday scan (REST-poll,
             */20 min 09:25-15:30, runs day_runner.one_pass incl. the 15:15+
             square-off sweep), 08:50 pre-market catch-up, hourly heartbeat,
             post-close swing scan, candle top-up + outcome evaluation,
             nightly universe, weekly retrain, nightly backup
  ui/        React app (see §11)
...
- **Theme (black + red, owner-requested rebrand 23-Jul-2026; supersedes the original violet scheme):** true-black neutral base (#050506 — no blue cast), glassmorphic cards (translucent red-tinted fill over the black base, 1px soft red ring, backdrop-blur). **Accent palette is a scarlet gradient:** red (#DC2626 → #EF4444 → #FCA5A5) is the primary brand accent and glow — used for the NEUROX logo/wordmark, active navigation, mode toggle, live/connection status, calibration curve, IC bars, and info highlights, with tasteful glow on buttons/active states/key numbers. Directional and outcome semantics stay legible against the brand red: mint-green (#34D399) for long / positive outcomes (target hit, gains, included), rose-pink (#FB7185 — pinker and lighter than brand scarlet, always in data/badge contexts, never chrome) for short / risk / negative outcomes (stopped, losses, excluded), and amber (#FBBF24 / #FDE68A) for warnings, cautions and pending states (periwinkle retired with the violet scheme — on a red base it read as neither info nor warning). Neutral text ramp is zinc (#ECECEE → #6B6B76), tabular mono numerals. No candy gradients on content, no fake badges — there is no "accuracy %" anywhere in the product.
- **Splash intro (added 23-Jul-2026):** on each full page load, a ~2s animated NEUROX wordmark (staggered letter reveal, red-gradient X, drawn red underline) fades into the dashboard. It is a pure overlay — the app mounts and fetches underneath, costing zero startup time — and collapses to a brief static fade under `prefers-reduced-motion`.
...
- **Model page:** active model card, ALL CV folds table, red flags rendered as prominent rose / amber banners, feature IC table, retrain history.
...
**Unattended cloud deployment (`deploy/`, added 10-Jul-2026):** V1 (predictions/paper-trading only) can run 24/7 on a single always-on Ubuntu 22.04 VPS instead of a local machine, amd64 or arm64/aarch64 alike (verified against Oracle Cloud's free-tier Ampere A1) — `deploy/server_setup.sh` installs it (default layout: user `ubuntu`, `/home/ubuntu/NEUROX`; both overridable via `NEUROX_USER`/`NEUROX_APP_DIR` for the older dedicated-service-account layout) and ends by starting both services and self-checking them (`systemctl is-active` + a local `curl`); `deploy/oneshot.sh` is a zero-prerequisite entry point that fetches the repo and hands off to it, for a box that doesn't have anything cloned yet. `deploy/systemd/*.service` keep the dashboard and scheduler running (`Restart=always`). The dashboard binds `127.0.0.1` only and is reached over a private Tailscale tunnel (`tailscale serve`) — never a public bind, enforced by `deploy/firewall.sh` denying the port publicly and by `tests/test_deploy.py`. Self-retraining cadence is `training.retrain_schedule` (`weekly`, Sat 10:00 IST, default; or `daily`, ~16:45 IST). Every scheduled job is wrapped so one failure logs and the scheduler keeps running (§17.11). **Outage fix (23-Jul-2026):** the scheduler itself now runs the in-market intraday scan (previously `day_runner` was a manual CLI nothing launched on a VPS — an unattended box produced no intraday signals at all), plus an 08:50 pre-market catch-up that re-pulls daily bars and rebuilds a stale universe after any missed evening, and an hourly heartbeat line (token validity, last bar date, universe/surveillance dates, signals today) so a stalled pipeline is visible in `journalctl` as stale dates, never as ambiguous silence. Token-needing jobs skip with a clean "no valid token" log line and self-heal on later runs instead of stalling the day — a missed morning re-auth costs that morning's scans, not the whole pipeline until someone notices. The SEBI daily-2FA requirement is never removed; `fyers.auto_login` (config, **off by default**) optionally automates completing it headlessly via TOTP+PIN at 06:00 IST — a real security trade-off (seed+PIN then live in `config.yaml` on the server), documented prominently in `SETUP.md` and left off unless explicitly opted into.

**Dashboard trust model (stated explicitly, not an oversight):** the FastAPI REST/`/ws` surface (`src/api/app.py`) defaults to zero application-level authentication — it relies on the network perimeter (Tailscale-private reachability + `ufw` denying the port publicly, or `127.0.0.1`-only on a local machine). This is a deliberate call for a single-user V1 decision-support tool with no order-placement surface: worst case of the perimeter being breached is read-only exposure of your own scanner/journal/model data, not capital loss. **Hardening headers are always sent** (nosniff, `X-Frame-Options: DENY`, a CSP scoped to the self-contained SPA, `Referrer-Policy: no-referrer`). **Optional app-level auth (T13, off by default):** `security.require_tailscale_identity: true` makes every request (and the `/ws` upgrade) require a `Tailscale-User-Login` header — which only `tailscale serve` on `127.0.0.1` can set, so it is unspoofable from off-tailnet — with `security.allowed_logins` an optional per-identity allowlist. If a future version adds multi-user access, real money movement, or exposes this beyond a private tunnel, turn this on (or add stronger auth) first — don't extend the no-auth default past this trust boundary.
