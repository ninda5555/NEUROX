...
3. **Surveillance filter:** download NSE's daily consolidated surveillance file (`REG_INDddmmyy.csv`, published on nseindia.com "All Reports"; ASM list also at nseindia.com/reports/asm). Match on symbol/ISIN; exclude every GSM stage and ASM (both ST-ASM and LT-ASM) stage ≥ 1 by default (configurable). NSE's site needs browser-like headers + cookie warm-up; cache the file locally per day; if download fails, reuse yesterday's list and flag staleness in the UI — never silently trade an unscreened universe.
