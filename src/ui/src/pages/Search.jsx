...
export default function Search() {
  ...
  const { data, error, loading } = useApi(() => getSearch(q), [q])
  ...
  return (
    <div>
      ...
      <div className="mb-3.5 flex items-center gap-3">
        ...
        <span className="text-xs text-dim">{loading ? '…' : `${rows.length} matches`}</span>
      </div>
      {error ? (
        <PageError message="Couldn't search." detail={error.message ? `HTTP ${error.message}` : String(error)} />
      ) : (
